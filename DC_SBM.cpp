...
int main(int argc, char* argv[]) {
    ...
    // // FIRST WAY: iterations done here

    Result temp_result[iterations], final_result;

    #pragma omp parallel for
    for(int t = 0; t < iterations; ++t) {
        GreedyAlgorithm ga(sbm.Y, k);
        temp_result[t] = ga.single_infer();
    }

    //prendo il migliore
    final_result.score = -std::numeric_limits<double>::infinity();
    for(int t = 0; t < iterations; ++t) {
        if (temp_result[t].score > final_result.score) {
            final_result = temp_result[t];
        }
    }


    // // SECOND WAY: iterations done later 

    // Result final_result;
    // GreedyAlgorithm ga(sbm.Y, k);
    // final_result = ga.infer(iterations);


    double nmi_score = calculateNMI(sbm.X, final_result.config);
